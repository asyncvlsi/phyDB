...
void StatsComponent::ComputeLocation(Rect2D<double>& rect, CompOrient orient, Point2D<int> location, Point2D<int> origin, Point2D<int> size) {
    int llx, lly, urx, ury;
    switch(orient) {
...
void Stats::ComputeRUDY() {
    ...
    int grid_x = (int) std::ceil( (double)(die_area.URX() - die_area.LLX()) / (double) Gcell_size );
    int grid_y = (int) std::ceil( (double)(die_area.URY() - die_area.LLY()) / (double) Gcell_size );
